		...
		<view class="header-fixed">
			...
			<view class="nav-menu">
				<view v-for="(item, index) in navItems" :key="index" class="nav-item"
					:class="{ active: currentNav === index }" @click="switchNav(index)">
					{{ item.name }}
				</view>
			</view>
		</view>
		...
		<view class="content-area">
			<scroll-view scroll-y class="article-list" @scrolltolower="loadMore" refresher-enabled
				...
				<view class="loading-state">
					...
					<text v-else-if="noMoreData">没有更多文章了</text>
...
<script setup>
	...
	const loadArticles = () => {
		...
		setTimeout(() => {
			...
			// 添加到文章列表
			articleList.value = [...articleList.value, ...pageData];
